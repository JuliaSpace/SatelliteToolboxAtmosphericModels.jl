...
```julia
AtmosphericModels.jr1971(instant::DateTime, ϕ_gd::Number, λ::Number, h::Number[, F10::Number, F10ₐ::Number, Kp::Number]) -> JR1971Output
AtmosphericModels.jr1971(jd::Number, ϕ_gd::Number, λ::Number, h::Number[, F10::Number, F10ₐ::Number, Kp::Number]) -> JR1971Output
```
...
!!! note
    ...
    Between 90 km and 100 km, this implementation integrates the barometric equation
    numerically instead of using the closed-form solution of the reference, which leads to
    a discontinuity of a factor of about 6 at 100 km (also present in GMAT). Hence, the
    densities in this region differ from those obtained with GMAT.

This function return an object of type `JR1971Output` that contains the following
fields:
