...
```julia
AtmosphericModels.jb2008(instant::DateTime, ϕ_gd::Number, λ::Number, h::Number[, F10::Number, F10ₐ::Number, S10::Number, S10ₐ::Number, M10::Number, M10ₐ::Number, Y10::Number, Y10ₐ::Number, DstΔTc::Number]) -> JB2008Output
AtmosphericModels.jb2008(jd::Number, ϕ_gd::Number, λ::Number, h::Number[, F10::Number, F10ₐ::Number, S10::Number, S10ₐ::Number, M10::Number, M10ₐ::Number, Y10::Number, Y10ₐ::Number, DstΔTc::Number]) -> JB2008Output
```
...
The model is valid only between 90 km and 3000 km, and the functions throw an
`ArgumentError` if the altitude is outside this range.

!!! note

    As in the reference implementation, the field `exospheric_temperature` of the output
    includes the geomagnetic variation (`DstΔTc`) but not the local solar time and latitude
    correction, which is applied to the temperature profile used to compute the densities.

These functions return an object of type `JB2008Output` that contains the
following fields:
