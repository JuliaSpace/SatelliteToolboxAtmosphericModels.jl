## Description #############################################################################
#
# This file defines the overload of the function
# `SatelliteToolboxAtmosphericModels.AtmosphericModels._jr1971_roots` to work with
# `ForwardDiff.Dual` numbers. This is required to compute the derivatives of the JR71 model
# using ForwardDiff.jl.
#
# The problem is that the root finding algorithm (`PolynomialRoots.roots`) is not compatible
# with `ForwardDiff.Dual`. To overcome this, we use `ImplicitDifferentiation.jl`.
#
############################################################################################
...
polynomial coefficients `x`. The roots are returned as a flat vector of real numbers (Re,
Im, Re, Im, ...).
"""
function _differentiable_roots_forward(x)
    ...
    # We need to sort the roots to have a canonical representation and avoid discontinuities
    # when the order changes. We sort by real part, then imaginary part.
...
This is the conditions function for the implicit function. It must evaluate to zero if `y`
is the correct set of roots for the polynomial with coefficients `x`.
"""
function _differentiable_roots_conditions(x, y, z)
    ...
    # The condition is that P(root) = 0 for each root. This gives us 4 complex equations,
    # which we split into 8 real equations.
    conditions_vec = vcat(
...
# Create the implicit function that computes the roots of a polynomial in a differentiable
# way.
...
This function calls the differentiable root finding algorithm and then post-processes the
results to return the values in the format expected by the JR71 model.
"""
function _jr1971_roots(p::AbstractVector{<:ForwardDiff.Dual})
    ...
    r = [
        roots_f[1] + im * roots_f[2],
        roots_f[3] + im * roots_f[4],
        roots_f[5] + im * roots_f[6],
        roots_f[7] + im * roots_f[8]
    ]
