...
# contains mutations (e.g. the pre-allocated Legendre matrix and the tuple updates in the
# vector AP path) that Zygote.jl cannot differentiate. Hence, we define a rule that
...
function ChainRulesCore.rrule(
    ...
    # Number of scalar inputs before the magnetic index.
    n_scalars = 6

    function nrlmsise00_pullback(Δ)
        # Compute the Jacobian of all output fields with respect to the inputs in a single
        # forward sweep. The magnetic index is appended to the scalar inputs so that both
        # the daily value and the 7-element vector are supported. Notice that we must not
        # forward the user-provided matrix `P` here since it cannot store the dual numbers
        # used by ForwardDiff.jl.
        x = vcat([jd, h, ϕ_gd, λ, F10ₐ, F10], ap)

        jac = ForwardDiff.jacobian(
            x -> begin
                ap_x = (ap isa Number) ? x[n_scalars + 1] : x[(n_scalars + 1):end]
        ...
        Δap = (ap isa Number) ? vjp[n_scalars + 1] : vjp[(n_scalars + 1):end]

        return (NoTangent(), vjp[1:n_scalars]..., Δap)
    end
